...
def check_pr():
    if sql.execute('SELECT * FROM products;').fetchall():
        ...
        return False
def change_pr(pr_name, new_value, attr=''):
    if attr == 'name':
        sql.execute('UPDATE products SET pr_name=? WHERE pr_name=?;',(new_value, pr_name))
    elif attr == 'des':
        sql.execute('UPDATE products SET pr_des=? WHERE pr_name=?;', (new_value, pr_name))
    elif attr == 'count':
        sql.execute('UPDATE products SET pr_count=? WHERE pr_name=?;', (new_value, pr_name))
    elif attr == 'price':
        sql.execute('UPDATE products SET pr_price=? WHERE pr_name=?;', (new_value, pr_name))
    elif attr == 'photo':
        sql.execute('UPDATE products SET pr_photo=? WHERE pr_name=?;', (new_value, pr_name))
